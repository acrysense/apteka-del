...
$basketItemsIDs=ArrayUtil::getKeyColumns(
    CatalogBasket::getItems(),
    'PRODUCT_ID'
);

$basketItems=ArrayUtil::groupByField(CatalogBasket::getItems(), 'PRODUCT_ID');




if(!empty($product=$basketItems[$arResult['ID']])):
//    Debug::dump($product);
    ?>
    <script>
        $(document).ready(function () {
            $(".btn_reserve_product_single").fadeOut(300, function () {
                $(".btn_reserve_product_single").text('Уже в корзине').attr('data-incart', true).addClass('btn-red')
                $(".btn_reserve_product_single").fadeIn(300);
            })

            $("#product_counter_<?=$arResult['ID']?>").val(<?=$product['QUANTITY']?>);
...
<script>
    $(document).ready(function () {
        ...
        <?if(!empty($_GET['from'])):?>
            console.info('aaa');
            console.info('<?=$_SERVER['HTTP_REFERER']?>');
            $("#link_return").attr('href', '<?=$_SERVER['HTTP_REFERER']?>#el_<?=$_GET['el_id']?>');
        <?endif;?>
    });
</script>

...
if($arResult['MESSAGE']['CODE'] == 'message_confirm'){
    $msgTitle = IM::getMessageTitle('SENDER_SUBSCR_NOTE_CONFIRM');
}
elseif($arResult['MESSAGE']['CODE'] == 'message_success'){
    $msgTitle = IM::getMessageTitle('SENDER_SUBSCR_NOTE_SUCCESS');
}
else{
    $msgTitle = GetMessage('subscr_form_response_'.$arResult['MESSAGE']['TYPE']);
}
...
<div class="bx-subscribe"  id="sender-subscribe">
    <div class="bx-subscribe-title"><?=IM::getMessageTitle('WINDOW_SUBSCRIBE')?></div>

    <? //echo '<pre>'; print_r($arResult); echo '</pre>';?>
    <?if(isset($arParams['RELEASE_TEMPLATE']) && intval($arParams['RELEASE_TEMPLATE']) > 0 && strlen(trim(IM::getMessageTemplate('WINDOW_SUBSCRIBE'))) > 0):?>
        <div class="bx-subscribe-text">
            <?=IM::getMessageTemplate('WINDOW_SUBSCRIBE')?>
        </div>
    <?endif;?>
...
?>
			...
			<div class="bx_subscribe_response_container">
				<table>
					<tr>
						...
						<td>
							<div style="font-size: 22px;"><?=$msgTitle?></div>
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		...
		<div class="bx-input-group">
			<input type="email" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>" data-rule-required="true" data-rule-email="true" data-msg-required="Это поле обязательно к заполнению" data-msg-email="Нужно правильно ввести email">
		</div>
		...
		<div class="bx_subscribe_submit_container">
			<button class="btn regBtn" id="bx_subscribe_btn_<?=$buttonId?>"><span><?=GetMessage("subscr_form_button")?></span></button>
		</div>
	</form>
<?
$frame->beginStub();
?>
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		...
		<div class="bx-input-group">
			<input type="email" name="SENDER_SUBSCRIBE_EMAIL" value="" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>" data-rule-required="true" data-rule-email="true" data-msg-required="Это поле обязательно к заполнению" data-msg-email="Нужно правильно ввести email">
		</div>
